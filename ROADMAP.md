> **Project status update — 2026-09-24:** PKG-INTERACT remains the accepted interaction foundation on `main`, including the accepted post-merge hardening from PR #29. Supervised Discord acceptance subsequently exposed a narrower cross-layer conversational-quality regression: ordinary greetings can still fall back to generic assistant phrasing even while canonical embodiment grounding works. This does **not** reopen accepted interaction safety/ledger semantics, but it opens a new **INTERACT/CORE live-quality repair gate** for natural, non-canned expression. PKG-DISCORD v1 transport is separately live accepted and ready to merge.

# Sofía Ada Lyra: 19-package delivery roadmap

**Planning revision:** 2026-09-24 (America/Chicago). **Status:** roadmap documentation on `main`; package implementation and deployment remain separately gated. PR #1 (CORE), roadmap reconciliation PR #4, PR #2 (INTERACT), and PR #29 (INTERACT post-merge hardening) are merged. PKG-DISCORD v1 has now passed supervised live owner-DM transport acceptance and is being integrated. INTERACT remains accepted as the semantic/safety foundation, with a new narrow INTERACT/CORE quality-repair gate opened for generic/canned live dialogue. The roadmap remains **19 packages**; Discord is a cross-package workstream, not package #20.
...
| 5 | **PKG-NET · Scoped networking and distributed operation** | Authenticated network routes and bounded remote capabilities | The narrow Discord private-DM transport route has supervised live acceptance. Broader remote-host NET work, DNS/TLS/redirect enforcement and Artemis acceptance remain. No general web/search grant. |
...
2. **Integrate PKG-MEM originals/provenance/privacy** against the merged conversation/source contracts.
...
4. **PKG-DISCORD v1 transport is live accepted and merging.** Preserve its fail-closed single-user/private-DM scope; do not hide SOCIAL or personality gaps inside the adapter.
