...
| 5 | **PKG-NET · Scoped networking and distributed operation** | Authenticated network routes and bounded remote capabilities | **Durable local admission gate accepted and merged via PR #11.** Durable node identity, exact endpoint approval, exact-scope grants/revocation, replay ledger, identity/endpoint-bound gateways, retirement coordination and `DurableRemoteControl` composition are on `main`; 57/57 focused tests and the full repository suite were reported passing before merge. Production peer-authenticated transport, DNS/TLS/proxy enforcement and live Artemis acceptance remain. No general web/search grant. |
...
| 8 | **PKG-OPS · Fleet operations, diagnostics, performance, and orchestration** | Cross-platform telemetry, trusted zero-touch enrollment/decommissioning, autonomous upkeep, configuration drift, workload placement/failover, and bounded maintenance | **Planned Waves 1–5 source implementation merged via PRs #99/#103.** `main` now contains fleet lifecycle/placement, machine/system telemetry bridges and history, authenticated enrollment evidence, desired-state drift, typed service/container/VM maintenance mapping, workload migration sequencing, durable leases/epochs/fencing, fail-closed promotion, recovery/update-ring policy and exact Sparks-only final decommission approval evidence. Production remote NET transport/agent execution, real cross-host telemetry/maintenance, RUN supervisor integration, measured migration/failover/restore and soak remain unaccepted. |
...
| 12 | **PKG-DEV · Self-improvement and engineering** | Evidence-linked diagnosis/proposal, approved bounded OpenCode execution, tests and rollback | **Planned Waves 1–5 source implementation merged via PRs #99/#103.** Base-SHA verification, exact scope/diff checks, detached Git worktrees, targeted tests, reviewed candidate patches, guarded rollback and separate apply/commit/push authorization are on `main`. Live OpenCode host execution and end-to-end self-tooling acceptance remain. |
...
| 16 | **PKG-KNOW · Documents, reference knowledge, and provenance** | Read trusted manuals, PDFs, code/docs, project notes and later approved web material; preserve source/version/provenance, freshness, citations and correction state | **Planned Waves 1–5 source implementation merged via PRs #99/#103.** Local and repository ingestion, durable provenance, version/supersession/invalidation state and provenance-first retrieval are on `main`. PDF/manual ingestion, richer semantic retrieval/citation ranges, privacy/audience integration and live authoring/upkeep acceptance remain. No general web grant. |
| 17 | **PKG-INTEGRATE · Applications, services, and tool adapters** | Typed integrations to Home Assistant, JMRI, GitHub, Docker/Portainer, Hyper-V, Cloudflare, databases, Ollama and future services; includes governed self-tooling from documentation | **Planned Waves 1–5 source implementation merged via PRs #99/#103.** Stronger typed schemas, capability/side-effect policy, durable exact-version activation, fsynced receipts, duplicate-invocation protection and an existing CapabilitySystem bridge are on `main`. Real service canary/health/version/rollback proof and the full KNOW→DEV→SAFE/VERIFY→activation loop remain. |
