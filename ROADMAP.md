...
| 6 | **PKG-UI · Clients, Discord adapter, voice, and workbench** | Authenticated interfaces to the same Sofía, delivery/renderer acknowledgments, accessible text fallback | **Windows text workbench accepted on `main` via PRs #115/#116; Sparks-only Discord DM remains live accepted for v1 transport.** Desktop chat/history, durable drafts, private workbench state, single-owner application worker, adaptive theme, chamfered HUD controls and reviewed Quick Tools are accepted. Voice, renderer/avatar viewport, mobile/web clients, proactive outbound UI and real generation cancellation remain separately gated. |
| 7 | **PKG-RUN · 24/7 lifecycle and supervision** | External service supervision, single active instance, restart/backoff, bounded periodic cognition, health/recovery | **Repository primitives accepted via PR #107.** `main` has bounded periodic opportunities, local singleton lease/fencing epochs, stale-owner rejection, host-neutral supervisor, readiness timeout, restart backoff/window limits and durable supervisor events; Windows gate **78 passed / 1 skipped**, followed by a reported green full suite. **Current candidate branch `feature/pkg-ops-run-act-evolve-control-plane` now wires RUN's canonical lease/periodic boundary into `SofiaApplication`/composition while remaining disabled by default; its application/control-plane gate passed 13/13 and the cross-package regression gate passed 144/144, followed by a reported green full suite.** It is still not an installed OS service. Independent watchdog, cross-host leadership/fencing, standby/failover, RTO/RPO and soak remain open. |
| 8 | **PKG-OPS · Fleet operations, diagnostics, performance, and orchestration** | Cross-platform telemetry, trusted zero-touch enrollment/decommissioning, autonomous upkeep, configuration drift, workload placement/failover, and bounded maintenance | **Strong source foundation on `main` via PRs #99/#103/#104.** Fleet cognition exposes read-only fleet/telemetry/placement/drift/migration planning, while exact-grant remote service/reboot/package/VM/container operations exist through the distributed transport. **The current control-plane candidate reuses that same canonical `OpsToolService` inside runtime composition and passed the 13/13 + 144/144 Windows gates plus a reported green full suite.** Zero-touch discovery→attestation→enrollment→one-time ACT notice, standing-policy upkeep orchestration, actual workload movement, real heterogeneous deployment, failover/restore and soak remain open. |
| 9 | **PKG-ACT · Goals, initiative, and outreach** | Evidence-based goals, spontaneous candidate reflection, opt-in outreach, quiet/busy/stop controls, bounded helpers | **Repository primitives accepted via PR #105.** Source-linked outreach eligibility, immutable recipient/channel binding, durable attempts, bounded retry/dedupe, acknowledged receipts and fail-closed `outcome_unknown` are on `main`; Windows gate **90 passed**, followed by a reported green full suite. **The current control-plane candidate wires ACT to canonical application state/INTERACT queue and now includes a disabled-by-default RUN→ACT scheduler; schedule acceptance passed 60/60 focused and 155/155 combined package regressions, followed by a reported green full suite.** Sender execution remains explicitly injected and inactive by default. **The proactive Discord sender boundary is now candidate-accepted with 37/37 focused + 171/171 combined regressions and a reported green full suite, including durable chunk receipts, exact binding-generation fencing and fail-closed `outcome_unknown`.** Live Discord event-loop attachment remains separately gated. |
...
| 14 | **PKG-EVOLVE · Governed evolution** | Reviewed preference/config evolution and separately protected identity/Constitution amendments | **Repository primitives accepted via PR #106.** Reversible reviewed preference/config revisions and independently authorized identity/Constitution apply/rollback with exact digests, backups and post-write verification are on `main`; Windows gate **62 passed**, followed by a reported green full suite. **The current control-plane candidate exposes EVOLVE through the canonical runtime only via explicitly supplied adapters and independent verifiers; no permissive verifier is installed. The candidate passed 13/13 + 144/144 Windows gates plus a reported green full suite.** Production SAFE approval/authentication remains open; no self-approval path exists and no production protected state was modified. |
