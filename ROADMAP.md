...
| **Mind** | **G: Personality Architecture** | Next substantive implementation batch; discovery active | Persistent, grounded personality model; relationship to identity and Constitution; contextual expression, persistence, evaluation, and LLM boundary. Personality must not fabricate capabilities or physical acts, replace canonical facts, or become a fixed gesture script. |
...
| **22: Distributed / Multi-Machine Sofía** | Central Sofía coordinating authenticated remote machines and bounded agents; preserve node identity, evidence, and per-node authority | **Planned** |
...
**Batch G: Personality Architecture.** Trace persistent personality source → context → assembler → engine/provider → observed response; separate deterministic contracts from probabilistic model behavior; implement and test a grounded personality boundary. Keep Batch F's outstanding live and performance evidence visible without blocking all new engineering work.
