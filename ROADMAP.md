...
> **PKG-ENVIRONMENT NWS extension candidate — 2026-09-26:** Sparks explicitly authorized a narrow National Weather Service provider before the general web/search gate. `feature/pkg-environment-nws-host-location` pins network access to HTTPS `api.weather.gov`, requires `environment.nws.read`, normalizes current station weather plus bounded forecast into the existing shared snapshot, and separates USER/home from HOST/server location. The NWS live canary passed against `nws:KGKY`. The candidate now also persists HOST/server location in Sofía's `state/machine-locations.json` keyed by stable discovered machine ID, projects bounded location metadata through machine inventory tools, and auto-loads the current host record at composition startup; process-local HOST variables remain an explicit override. Because this persistence slice was added after the prior 85-test/full-suite evidence, fresh focused and full-suite acceptance are still required before merge. General browsing/search and arbitrary geocoding remain closed.
...
| 20 | **PKG-ENVIRONMENT · Time, location, season, weather, and ambient context** | One provenance-aware shared environment snapshot for cognition and package consumers | **Accepted offline foundation merged via PR #110.** `main` owns shared clock/environment projection, configured/current location evidence, timezone/DST, season/daylight, provider-neutral weather/forecast/indoor observations, freshness/provenance, deterministic queries, HA bridge and AVATAR consumption. **Current candidate:** NWS weather/forecast pinned to `api.weather.gov` under `environment.nws.read`, plus persistent HOST/server location keyed to stable machine identity. Initial 85-test/full-suite and live NWS canary passed before the persistence slice; fresh focused/full-suite regression is required for the final head. General browsing/search and arbitrary geocoding remain closed. See [ENVIRONMENT readiness](docs/development/pkg-environment-readiness-roadmap.md). |
