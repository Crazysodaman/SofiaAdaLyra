...
| 7 | **PKG-RUN · 24/7 lifecycle and supervision** | External service supervision, single active instance, restart/backoff, bounded periodic cognition, health/recovery | **Local lifecycle source implementation merged via PR #107.** `main` now has bounded periodic opportunities, a durable local singleton lease with fencing epochs, stale-owner rejection, host-neutral supervisor, readiness timeout, restart backoff/window limits, and durable supervisor events. Windows offline gates passed **78 tests with 1 skip**. Post-merge current-`main` full repository pytest was reported passing by Sparks; aggregate count was not supplied. Real OS service installation, independent watchdog, standby/failover, RTO/RPO and soak remain open. |
...
| 9 | **PKG-ACT · Goals, initiative, and outreach** | Evidence-based goals, spontaneous candidate reflection, opt-in outreach, quiet/busy/stop controls, bounded helpers | **Durable outreach/delivery source implementation merged via PR #105.** `main` now has source-linked outreach eligibility, immutable recipient/channel binding, durable delivery attempts, bounded retry/dedupe, acknowledged receipts, and fail-closed `outcome_unknown` handling. Windows offline gates passed **90 tests**. Post-merge current-`main` full repository pytest was reported passing by Sparks; aggregate count was not supplied. Real sender/channel activation and RUN-triggered scheduling remain separately gated. |
...
| 14 | **PKG-EVOLVE · Governed evolution** | Reviewed preference/config evolution and separately protected identity/Constitution amendments | **Governed revision source implementation merged via PR #106.** `main` now has reversible reviewed preference/config revisions plus independently authorized identity/Constitution amendment apply/rollback with exact digests, backups and post-write verification. Windows disposable/offline gates passed **62 tests**. Post-merge current-`main` full repository pytest was reported passing by Sparks; aggregate count was not supplied. No self-approval path exists; no production protected state was modified. |
