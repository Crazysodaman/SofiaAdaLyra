...
> **PKG-ENVIRONMENT NWS extension candidate — 2026-09-26:** Sparks explicitly authorized a narrow National Weather Service provider before the general web/search gate. `feature/pkg-environment-nws-host-location` pins network access to HTTPS `api.weather.gov`, requires `environment.nws.read`, normalizes current station weather plus bounded forecast into the existing shared snapshot, and adds an independent configured HOST/server location so Sofía's runtime location can differ from Sparks' USER/home location. This is not accepted until Windows focused/full-suite and supervised live NWS canary evidence pass. General browsing/search and arbitrary geocoding remain closed.
