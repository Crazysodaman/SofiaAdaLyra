...
| 20 | **PKG-ENVIRONMENT · Time, location, season, weather, and ambient context** | One provenance-aware shared environment snapshot for cognition and package consumers | **Accepted offline foundation merged via PR #110.** `main` now owns shared clock/environment projection, configured-vs-current USER/SITE/HOST location evidence, timezone/DST, hemisphere-aware season/daylight, provider-neutral weather/forecast/indoor observations, freshness/provenance, deterministic direct queries, bounded environment-source reporting, an explicitly authorized Home Assistant bridge, and AVATAR consumption. Windows gates: **79 focused**, **61 touched regression**, **27 provenance regression**, plus reported green full suites and supervised live Ollama acceptance. Next: optional live HA canary after explicit `environment.home_assistant.read`; direct internet weather/geocoding remains behind NET/web authorization. See [ENVIRONMENT readiness](docs/development/pkg-environment-readiness-roadmap.md). |
