...
- **Current:** Waves 1–5 plus toolbox/fleet-transport source completion are merged via PRs #99/#103/#104. Fleet lifecycle/placement, machine/system telemetry, machine inventory cognition, fleet status/telemetry, placement/drift/migration planning, typed local/remote maintenance and the pinned mTLS remote agent/transport are repository accepted. The current control-plane candidate reuses the same canonical `OpsToolService` in runtime composition and passed **13/13 + 144/144** Windows gates plus a reported green full suite. Real heterogeneous-host deployment, workload execution, standing-policy maintenance, failover and soak proof remain open.
...
- **Current:** **durable ACT outreach/delivery source implementation is merged via PR #105.** `main` now includes source-linked outreach eligibility, immutable recipient/channel binding to INTERACT queued messages, durable send attempts, bounded retry/dedupe, acknowledged delivery history and fail-closed `outcome_unknown` handling. Windows offline acceptance passed **90 tests**. Post-merge current-`main` full repository pytest was reported passing by Sparks; aggregate count was not supplied. The current control-plane candidate binds ACT to canonical application state/INTERACT queue and adds disabled-by-default RUN→ACT scheduling while preserving explicit injected senders and no default delivery; scheduler acceptance passed **60/60 focused** and **155/155 combined package**, followed by a reported green full suite. The proactive Discord sender boundary is candidate-accepted with **37/37 focused + 171/171 combined** and a reported green full suite, preserving exact owner/channel/session/generation checks, durable chunk receipts and fail-closed uncertain outcomes. No live event-loop activation is claimed.
