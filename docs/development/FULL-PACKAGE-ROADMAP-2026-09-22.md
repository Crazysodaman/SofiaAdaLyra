> **Project status update — 2026-09-26:** PKG-ENVIRONMENT merged to `main` via PR #110 (`766bf21e`) after Windows acceptance: **79 focused**, **61 touched-regression**, and **27 provenance-regression** tests passed, Sparks reported green full-suite runs before and after the provenance repair, and supervised live Ollama checks passed for time/location/season/daylight/weather-unavailable/provenance behavior. The optional live Home Assistant canary and direct internet weather/geocoding remain separately gated. Earlier DEV/KNOW/INTEGRATE/OPS source acceptance remains unchanged; none of this is proof of production remote NET transport, deployed multi-host orchestration, RUN 24/7 supervision, live failover or soak.
...
- **Current:** **accepted offline foundation merged via PR #110.** `main` now provides one shared provider-neutral `EnvironmentSnapshot`, trusted runtime-clock projection, configured-vs-current USER/SITE/HOST location semantics, ZoneInfo/DST-safe local time, hemisphere-aware season/daylight, bounded weather/forecast/indoor observations with freshness/provenance, deterministic direct queries including bounded environment-source reporting, an explicitly authorized Home Assistant bridge, runtime cognition integration and AVATAR consumption. Windows gates passed **79 focused**, **61 touched-regression**, and **27 provenance-regression** tests; Sparks reported green full suites and supervised live Ollama acceptance.
- **Build:** next work is provider activation rather than rebuilding the offline foundation: optionally canary explicitly configured Home Assistant weather/indoor/current-location entities under `environment.home_assistant.read`, then later add a separately authorized narrow remote weather/geocoding provider only after the NET/web gate. Preserve source timestamps, freshness, subject isolation, privacy and no competing consumer-owned current-state logic.
...
13. **ENVIRONMENT provider activation:** the offline foundation is accepted on `main`; next, optionally canary Home Assistant through INTEGRATE with explicit `environment.home_assistant.read`, then leave direct internet weather/geocoding for the later separately authorized NET/web gate.
