...
9. INTERACT will later consume WardrobeTextProjection rather than infer clothing from prose.
10. MEM may later store history/preferences, but memory is never current worn-state authority.

## Next work, dependency order

1. **Real 3D base and provenance:** obtain/review a suitable editable Blender humanoid base, pin license/source, shape it against canonical measurements and approved reference, save an inspected .blend, and keep the old procedural meshes only as technical fixtures.
2. **A1 actual continuous adult female sculpt:** create the final body topology and approved silhouette, face, hands, feet, fox ears/tail and required external anatomy in the controlled authoring model. Verify real mesh measurements and front/side/back views.
3. **A1c rig and real anchors:** deformable body, hands/fingers, eyes/face/ears, independent tail bones, actual garment anchor transforms and pose tests.
4. **A2 properly fitted garments:** underlayers, independently verified fully clothed fallback, engineer outfit and lounge clothes all on the same body/rig. Test coverage, opacity, collision, tail/ear openings and motion.
5. **A3 renderer + shared-state adapter:** implement authenticated renderer receipts that drive SharedWardrobeState, hide stale avatar revisions, expose the compact text projection to INTERACT, and verify avatar/text remain one state across success, failure, fallback, restart and resync.
6. **A3 privacy/reliability:** no accidental authoring-base inclusion/fallback, default-denied restricted previews, thumbnail/cache/crash/stop tests and resource budgets.
7. **A4/A5 later:** art-backed virtual props, MEM-persisted sourced wardrobe choices, safe preset-based outfit proposals and supervised DEV/SAFE Blender generation. No general internet or unsupervised host Python/hardware authority.
