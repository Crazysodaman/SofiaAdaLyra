...
3. **Review hardening scope and base integration:** original PKG-INTERACT PR #2 was already accepted and merged to `main` at `d6658d0`. The current acceptance target is the post-merge `fix/interact-live-behavior-hardening` branch. Current comparison shows the hardening branch ahead of `main` and not behind it. Optional interaction tables remain intentionally not auto-created by normal startup/read paths; any future production provisioning is a separate reviewed migration before staged offers are enabled.
4. **Human acceptance and separate integration approval:** Sparks reviews the post-merge hardening dialogue, architecture and known limitations, explicitly accepts the hardening candidate, and separately authorizes any integration into `main`. Acceptance alone does not merge anything.
