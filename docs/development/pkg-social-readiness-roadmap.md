> **Live finding — 2026-09-24:** supervised PKG-DISCORD v1 acceptance proved the exact enrolled owner can be authenticated and isolated at the transport boundary, but that authenticated principal is not yet projected into shared cognition as `Sparks`. This is now the minimum SOCIAL implementation gate. Discord must not hard-code a name or relationship to conceal the missing shared principal layer.
