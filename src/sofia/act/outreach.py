"""Side-effect-free outreach eligibility. No network, scheduler or sending."""
...
class Candidate:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.recipient_id, str) or not self.recipient_id.strip():
        ...
        if not isinstance(self.evidence_ids, tuple) or not self.evidence_ids or any(
            ...
            raise ValueError("nonempty evidence_ids tuple required")
...
class Policy:
    ...
    def __post_init__(self) -> None:
        ...
        if any(type(value) is not int or not 0 <= value <= 23 for value in (self.quiet_start_utc, self.quiet_end_utc)):
...
@dataclass(frozen=True)
class History:
    """Trusted host-supplied, acknowledged deliveries only, not queued attempts."""
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.delivered_candidate_ids, frozenset) or any(
            not isinstance(item, str) or not item.strip() for item in self.delivered_candidate_ids
        ):
            raise ValueError("delivered_candidate_ids must be a frozenset of IDs")
        ...
        if self.delivered_day_utc is not None:
            try:
                date = datetime.strptime(self.delivered_day_utc, "%Y-%m-%d").date()
            except (ValueError, TypeError) as exc:
                raise ValueError("delivered_day_utc must be ISO date") from exc
            if date.isoformat() != self.delivered_day_utc:
...
def evaluate(candidate: Candidate, policy: Policy, history: History, now: datetime, *, busy: bool = False) -> Decision:
    """Eligibility is a *proposal*, never permission or confirmation of delivery."""
    ...
    if policy.quiet_start_utc == policy.quiet_end_utc:
        quiet = True  # fail closed: equal start/end is an all-day quiet period
    ...
    if history.delivered_day_utc == moment.date().isoformat() and history.delivered_today >= policy.max_daily:
