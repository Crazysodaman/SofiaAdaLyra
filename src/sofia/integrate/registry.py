...
class AdapterRegistry:
    def __init__(self)->None: self._adapters:dict[str,Adapter]={}
    def register(self,adapter:Adapter)->None:
        manifest=adapter.manifest
        old=self._adapters.get(manifest.tool_id)
        if old is not None and old.manifest.version != manifest.version: raise AdapterRegistrationError("tool_id already registered at another version")
        self._adapters[manifest.tool_id]=adapter
    def manifest(self,tool_id:str)->AdapterManifest|None:
        a=self._adapters.get(tool_id); return None if a is None else a.manifest
    def tool_ids(self)->tuple[str,...]: return tuple(sorted(self._adapters))
    def invoke(self,request:ToolInvocation)->ToolReceipt:
        ...
        return ToolReceipt(request.invocation_id,request.tool_id,adapter.manifest.version,started,finished,ok,output,error)
