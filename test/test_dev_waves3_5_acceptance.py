...
def _repo(tmp_path:Path)->tuple[Path,str]:
    ...
    _git(root,"config","user.name","Sofia Tests")
    (root/"src").mkdir()
    (root/"src"/"allowed.py").write_text("VALUE = 1\n",encoding="utf-8")
    (root/"state").mkdir()
    (root/"state"/"sofia.db").write_bytes(b"baseline")
    _git(root,"add",".")
...
def test_git_workspace_detects_out_of_scope_change(tmp_path:Path):
    ...
    (root/"outside.txt").write_text("nope",encoding="utf-8")
    git=GitWorkspace(root)
    git.require_head(sha)
    with pytest.raises(ChangeScopeError):
        git.require_changes_within(("src",))
...
def test_apply_commit_and_push_have_separate_authority_and_preserve_runtime_state(tmp_path:Path):
    ...
    with pytest.raises(PermissionError):
        flow.push("main",authorized=False)
...
def test_rollback_only_reverts_unchanged_applied_candidate(tmp_path:Path):
    ...
    flow.apply(candidate,authorized=True)
    with pytest.raises(PermissionError):
        flow.rollback_applied(authorized=False)
